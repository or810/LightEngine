...
namespace Light {
	...
	template<typename T>
	class QueueSequenceStrategy : public ISequenceManagementHandler<T, std::greater<T>> {
	protected:
		QueueSequenceStrategy() : m_count(0) {
			// Automatically use std::greater comparator
			this->m_tree = std::make_unique<BinarySearchTree<T, std::greater<T>>>();
		}
