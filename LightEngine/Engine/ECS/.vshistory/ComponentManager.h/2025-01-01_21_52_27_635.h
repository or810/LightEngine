...
namespace Light {
	...
	public:
		...
		void add(Entity entity, T& component) 
		{
			...
			//m_entitySignatures[entity]->addComponent<typeid(T)>
		}
		...
		void add(Entity entity, T&... components)
		{
			...
			(void)((getComponentPool<T>()->insertComponent(entity, components), ...));
			std::shared_ptr<Signature> signature = std::make_shared<Signature>();
			(void)((signature->addComponent<T>(), ...)); // Correct fold expression
