...
constexpr auto MAX_COMPONENTS = 64;
    ...
    public:
        ...
        Signature() 
            : m_signature()
        {
            (m_signature.set(m_typeToIndex[std::type_index(typeid(Components))]), ...);
        }

        template<typename Component>
        void addComponent() 
        {
            m_signature.set(m_typeToIndex[std::type_index(typeid(Component))]);
        }

        template<typename Component>
        void removeComponent()
        {
            m_signature.reset(m_typeToIndex[std::type_index(typeid(Component))]);
        }

        template<typename Component>
        void flipComponent()
        {
            m_signature.flip(m_typeToIndex[std::type_index(typeid(Component))]);
        }

        template<typename Component>
        bool has()
        {
            return m_signature.test(m_typeToIndex[std::type_index(typeid(Component))]);
        }
        
        template<typename... Components>
        bool has()
        {
            return ((m_signature.test(m_typeToIndex[std::type_index(typeid(Components))]) && ... ) ;
        }
