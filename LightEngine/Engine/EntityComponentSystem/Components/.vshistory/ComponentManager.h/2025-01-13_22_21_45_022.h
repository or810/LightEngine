...
namespace Light {
	namespace ECS {
		...
		public:
			ComponentManager(size_t initialPoolSize) : allocator(initialPoolSize), m_initialPoolSize(initialPoolSize) {}
			...
			void registerComponent() {
				...
				if (componentPools.find(id) == componentPools.end()) {
					componentPools[id] = allocator.allocateObject<ComponentPool<T>>(m_initialPoolSize);
				}
			}
