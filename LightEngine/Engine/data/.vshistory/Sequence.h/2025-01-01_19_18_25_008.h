...
namespace Light {
	...
	public:
		...
		void pop(std::shared_ptr<T> element) {
			this->remove(element); // Removing by element itself
			m_count--;
		}

		PriorityQueue<T>& operator<<(const T& element) {
			auto sharedPtr = std::make_shared<T>(element);  // Creating shared pointer for element
			push(sharedPtr);  // Push it into the queue
			return *this;
		}
