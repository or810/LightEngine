...
void operator delete(void* pointer) noexcept {
    ...
    totalDeallocations += sizeof(*pointer);  // Track deallocation size (not exact, but works for simple types)
    std::cout << "Deallocated memory. Total deallocations: " << totalDeallocations << " bytes.\n";
}
